...
def index(request):
    ...
    attributions=len(Mouvement.objects.all())
    p_panne= int(pannes*100 / Ordinateurs)
    P_ut= int(attributions * 100 / Ordinateurs)
    P_lic=int(licences * 100 / Ordinateurs)
    dashboard={
...
def panne(request):
    ...
    Pannes_hist =Histo.objects.all()
    return render(request,'list_panne.html',{'Pannes':Pannes,'Pannes_hist':Pannes_hist})
...
def modifier_personne(request, id):
    ...
    if request.method == 'POST':
        ...
        personne.departement= request.POST['departement']
        personne.poste = request.POST['poste']
...
@login_required
def repare(request,pk):
    panne = get_object_or_404(Panne, pk=pk)
    Pannes=Histo.objects.all()
    intitule=panne.intitule
    utilisateur=panne.utilisateur
    telephone=panne.telephone
    marque=panne.marque
    departement=panne.departement
    pan=Histo(intitule=intitule,utilisateur=utilisateur,telephone=telephone,marque=marque,departement=departement)
    pan.save()
    panne.delete()
